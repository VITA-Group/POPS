...
def split_and_save_dataset(dataset, save_dir, split_ratios):
    ...
    for forget_percentage in split_ratios:
        ...
        if os.path.exists(forget_path) and os.path.exists(retain_path):
            print(f"Split {forget_percentage}% already exists, skipping.")
            continue

        print(f"Creating split: forget {forget_percentage}%, retain {retain_percentage}%")
        ...
        forget_ids = [dataset[idx]["ID"] for idx in forget_indices]
        retain_ids = [dataset[idx]["ID"] for idx in retain_indices]
        ...
        print(f"Saved {forget_percentage}% split to {forget_dir}")
        ...
        for step, batch in enumerate(progress_bar_forget):
            ...
            if args.gradient_accumulation:
                with accelerator.accumulate(model):
                    ...
                    loss_forget = -(outputs.loss / accumulation_steps)                                     
        ...
        for step, batch in enumerate(progress_bar_retain):
            ...
            if args.gradient_accumulation:
                with accelerator.accumulate(model):
                    ...
                    loss_retain = (outputs_current.loss + kl_div) / accumulation_steps                                      
